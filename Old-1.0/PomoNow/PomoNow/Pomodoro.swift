...
//        isDebug = true //调试模式
    ...
    func start() {
        if pomoMode == 0 {
            ...
            //数据记录
            Answers.logCustomEventWithName("A Pomodoro",customAttributes: [:])
        }
    }
    ...
    private func breakStart() {
        ...
        taskChanged = true
        //数据记录
        Answers.logCustomEventWithName("completed Pomodoro",customAttributes: [:])
    }
