...
class PomodoroViewController: UIViewController , UINavigationControllerDelegate{
        ...
        if pomodoroClass.pomoMode == 0 {
            if sender.state == UIGestureRecognizerState.Ended {
                ...
                if process < 100 {
                    ...
                    pomodoroClass.start()
                    stopTimer()
                    timer = NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: #selector(PomodoroViewController.pomoing(_:)), userInfo: nil, repeats: true)
                }
    ...
    func processToZero() { //更新进度条状态
        aniMode = true
        needStop = true
        timer = NSTimer.scheduledTimerWithTimeInterval(0.01, target: self, selector: #selector(PomodoroViewController.processAnimation(_:)), userInfo: nil, repeats: true)
    }
